...
namespace Coherence.Toolkit
{
    ...
    public class CommandsHandler
    {
        ...
        private bool ValidateReflectedCommandSending(string commandName, MessageTarget messageTarget, SendOptions options, out List<CommandBinding> bindings)
        {
            ...
            if (commandName.Length > OutProtocolBitStream.SHORT_STRING_MAX_SIZE)
            {
                logger.Warning(Warning.ToolkitCommandSendReflectedValidation,
                    $"Trying to send command '{commandName}' with too many characters in its name ({commandName.Length}, maximum is {OutProtocolBitStream.SHORT_STRING_MAX_SIZE}.) This message will not be sent. {sync}");
                return false;
            }
