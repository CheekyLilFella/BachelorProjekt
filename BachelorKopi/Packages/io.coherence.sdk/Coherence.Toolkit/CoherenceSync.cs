...
namespace Coherence.Toolkit
{
    ...
    public sealed partial class CoherenceSync : CoherenceBehaviour, ICoherenceSync, IConnectedEntityDriver
    {
        ...
        /// <summary>
        /// <see langword="true"/> when <see cref="CoherenceNode.path"/> changes.
        /// </summary>
        /// <seealso cref="CoherenceNode"/>
...
#pragma warning restore CS0618
        ...
        private bool ConnectToBridge()
        {
            ...
            {
                logger = Log.GetLogger<CoherenceSync>(gameObject.scene);
                logger.Warning(Warning.ToolkitSyncMissingBridgeDisabling,
                    ("instance", name));
        ...
        private void HandleConnected(ICoherenceBridge coherenceBridge)
        {
            ...
            if (resolvedEntity.entityData.HasValue)
            {
                ...
                ((ICoherenceSync)this).ApplyNodeBindings();

                if (updater.ChangedConnection)
                {
                    ((ICoherenceSync)this).ConnectedEntityChanged(updater.NewConnection);
                }
            ...
            // It's possible one of the callbacks cleared the state?
            // Authority_AutoAdoption_Simulator_ServerSideWithClientInput fails without
            // this check.
            if (EntityState != null)
            {
                // In some situations like nested CS prefabs we need to ensure after this entity is created
                // that entities depending on this creation are instantiated.
                currentBridge.EntitiesManager.InstantiateDelayedEntitiesDependingOnThisParent(EntityState.EntityID);
            }
        }
        ...
        // fixme: Should be internal, blocked by baked sync scripts.
        [EditorBrowsable(EditorBrowsableState.Never)]
        void ICoherenceSync.ConnectedEntityChanged(Entity newConnectedEntityID)
        {
            var oldParent = ConnectedEntity == null ? null : tform.parent;

            ConnectedEntity = newConnectedEntityID == Entity.InvalidRelative ? null : bridge.GetCoherenceSyncForEntity(newConnectedEntityID) as CoherenceSync;

            logger.Trace($"ConnectedEntityChange: {EntityState.EntityID} -> {newConnectedEntityID} = {ConnectedEntity.SelfOrNull()?.EntityState.EntityID ?? Entity.InvalidRelative}");
