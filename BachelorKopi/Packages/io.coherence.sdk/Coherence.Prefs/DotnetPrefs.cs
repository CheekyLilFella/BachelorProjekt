...
namespace Coherence.Prefs
{
    ...
    public class DotnetPrefs : IPrefsImplementation
    {
        ...
        private Dictionary<string, PrefsObject> prefsByKey = new Dictionary<string, PrefsObject>();
 ...
 prefsFilePath ?? Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Prefs", "prefs.bin"); // Based on the exec path
        ...
        public void Save()
        {
            // If nothing to save just delete the prefs file
            if (prefsByKey.Count <= 0)
            {
                try
                {
            ...
            ArraySegment<byte> prefsBinary = SerializePrefs();
            using (FileStream fileStream = File.Open(prefsFilePath, FileMode.Create))
            {
        ...
        private void Load()
        {
            try
            {
                ...
                byte[] prefsBinary = File.ReadAllBytes(prefsFilePath);
